def read_data(username, password, classcode):
  ...
  if username_validator(username, password):
    df = pd.read_csv(x + '/static/ALC_Class_Report_Test.csv')
    ...
    cols = [
      'Parent Name', 'Student Name', 'AvatarGrade', 'Teacher', 'Zoom',
      'Parent phone', 'ClassCode'
    ]
...
def username_validator(username, password):
  ...
  x = os.getcwd()
  df = pd.read_csv(x + '/static/username_validator.csv')
...
def send_sms(name, num, account_sid, auth_token):
  ...
  message = client.messages \
      .create(
           body= f'Dear {name}, this is from Avatar Learning Center. Your class has started, please join. Please contact us at 515-428-2827 to reply.',
           from_='+18777432015',
           to=num
       )
...
def classcode_validator(username, classcode):
  ...
  import os
  x = os.getcwd()
  df = pd.read_csv(x + '/static/classcode_validator.csv')
  df = df[df['username'] == username]
